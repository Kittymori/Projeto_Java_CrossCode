...
public class TelaRegistroEventoSentinela extends JFrame {
    ...
    public void initUI() {
       ...
       grid.gridx = 0;
        ...
        grid.gridx = 0;
        grid.weighty = 1.0;
        panelContent.add(createEventoSentinelaPanel(fonteLabel, fonteCampo), grid);
    ...
    private JPanel createEventoSentinelaPanel(Font fonteLabel, Font fonteCampo) {
        ...
        panel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createLineBorder(Cores.COR_LETRA_PAINEL.brighter()),
                "Tipo de Evento Sentinela",
                TitledBorder.LEFT, TitledBorder.TOP,
                fonteLabel, Cores.COR_LETRA_PAINEL
        ));

        grupoEventos = new ButtonGroup();
        String[] eventos = {
                "Tentativa de Suicídio", "Desnutrição", "Quedas",
                "Óbito", "Diarreia", "Pressão arterial", "Escabiose",
                "Glicemia", "Desidratação", "Temperatura", "Úlcera por pressão"
        };

        for (String evento : eventos) {
            JRadioButton radio = new JRadioButton(evento);
            radio.setFont(fonteCampo);
            grupoEventos.add(radio);
            panel.add(radio);
        }
